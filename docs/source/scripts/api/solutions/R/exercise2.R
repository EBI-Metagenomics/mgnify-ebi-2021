...
colnames(access.df) = c("Phylum", "Rel.Ab", "Accessions")

# determine most common phyla across accessions
summ = aggregate(Rel.Ab ~ Phylum, data=access.df, sum)
...
for (b in unique(access.df$Accessions)){ # for each accession
  relab.other = 100-sum(as.numeric(access.df[which(access.df$Accessions == b & access.df$Phylum %in% top.phylum),"Rel.Ab"])) # take relative abundance for all non-top phyla
  access.df = rbind(access.df, c("Other", relab.other, b))
}
access.df = access.df[which(access.df$Phylum %in% top.phylum | access.df$Phylum == "Other"),] # only keep top phyla and the other category
access.df$Phylum = factor(access.df$Phylum, levels=rev(c(top.phylum, "Other"))) # reorder phylum for plotting
