...
**Please note**: this step might fail due to insufficient resources. If this happens, please use the output we have generated earlier with the same command:

.. code-block:: bash

    /opt/data/checkm_answers
...
.. code-block:: bash
    ...
    nw_rename /opt/data/checkm_answers/concatenated.tre /opt/data/assemblies/rename_list.tab > /opt/data/assemblies/renamed.tree
...
In order to open **FigTree** open a new terminal window (without docker) and run the following commands:  
